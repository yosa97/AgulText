...
# Model default: 0.5B (cepat, cocok untuk smoke test).
# Untuk meniru tournament lebih realistis, gunakan model 5-9B:
#   MODEL=Qwen/Qwen2.5-7B-Instruct bash examples/run_instruct.sh
#   MODEL=tokyotech-llm/Llama-3-Swallow-8B-v0.1 bash examples/run_instruct.sh
MODEL="${MODEL:-Qwen/Qwen2.5-0.5B-Instruct}"
HF_TOKEN="${HF_TOKEN:-}"
HF_REPO="${HF_REPO:-}"
# Tournament text tasks biasanya 0.75–1.5 jam.
# Default 0.75 jam untuk meniru kondisi tournament paling ketat.
HOURS="${HOURS:-0.75}"
...
try:
    ...
    for item in raw:
        ...
        if not out or len(out) < MIN_OUT_CHARS:
            continue
        # Gunakan field name "instruct" dan "input" — sama persis dengan
        # yang dikirim validator tournament (bukan "instruction").
        samples.append({"instruct": instr, "input": inp, "output": out})
        if len(samples) >= N_TARGET:
            break

    with open(DATASET_PATH, "w", encoding="utf-8") as f:
        json.dump(samples, f, ensure_ascii=False)
    print(f"Download berhasil: {len(samples)} samples dari Stanford Alpaca (field: instruct/input/output)")
    sys.exit(0)
...
[
  {
    "instruct": "Jelaskan secara mendalam bagaimana arsitektur Transformer bekerja dalam deep learning.",
    "input": "",
  ...
  {
    "instruct": "Bagaimana algoritma backpropagation bekerja dalam melatih neural network?",
    "input": "",
  ...
  {
    "instruct": "Jelaskan perbedaan antara supervised, unsupervised, dan reinforcement learning secara mendalam.",
    "input": "",
  ...
  {
    "instruct": "Apa itu attention mechanism dan bagaimana kontribusinya dalam pemrosesan bahasa alami?",
    "input": "",
  ...
  {
    "instruct": "Jelaskan berbagai teknik regularisasi yang digunakan untuk mencegah overfitting dalam deep learning.",
    "input": "",
  ...
  {
    "instruct": "Bagaimana cara kerja optimizer Adam dan mengapa ia lebih unggul dibandingkan gradient descent biasa?",
    "input": "",
  ...
  {
    "instruct": "Jelaskan konsep transfer learning dan bagaimana pra-pelatihan digunakan dalam model bahasa modern.",
    "input": "",
  ...
  {
    "instruct": "Apa itu word embedding dan bagaimana cara kerjanya dalam merepresentasikan makna kata?",
    "input": "",
  ...
  {
    "instruct": "Jelaskan perbedaan mendasar antara CNN dan RNN serta kapan sebaiknya menggunakan masing-masing.",
    "input": "",
  ...
  {
    "instruct": "Bagaimana model BERT bekerja dan apa yang membuatnya berbeda dari model bahasa sebelumnya?",
    "input": "",
  ...
  {
    "instruct": "Jelaskan teknik kuantisasi model AI dan bagaimana ia membantu deployment model besar.",
    "input": "",
  ...
  {
    "instruct": "Apa itu RLHF dan bagaimana teknik ini digunakan untuk melatih model bahasa seperti ChatGPT?",
    "input": "",
  ...
  {
    "instruct": "Jelaskan arsitektur Mixture of Experts dan keunggulannya dalam scaling model bahasa besar.",
    "input": "",
  ...
  {
    "instruct": "Bagaimana algoritma BPE tokenisasi bekerja dan mengapa ia penting dalam model bahasa modern?",
    "input": "",
  ...
  {
    "instruct": "Jelaskan konsep positional encoding dalam Transformer dan perbedaan berbagai pendekatannya.",
    "input": "",
  ...
  {
    "instruct": "Apa perbedaan antara full fine-tuning dan parameter-efficient fine-tuning, serta kapan menggunakan masing-masing?",
    "input": "",
  ...
  {
    "instruct": "Jelaskan cara kerja Flash Attention dan mengapa ia penting untuk training model dengan konteks panjang.",
    "input": "",
  ...
  {
    "instruct": "Apa itu KV cache dalam inferensi model bahasa dan bagaimana ia meningkatkan efisiensi generasi teks?",
    "input": "",
  ...
  {
    "instruct": "Jelaskan paradigma Retrieval Augmented Generation dan bagaimana ia mengatasi keterbatasan model bahasa.",
    "input": "",
  ...
  {
    "instruct": "Jelaskan scaling laws dalam AI dan bagaimana ia mempengaruhi keputusan dalam pengembangan model besar.",
    "input": "",
...
# 7. Training masuk SUCCESS path (bukan failure/fallback path)
# "Training successfully done" = success.txt ditulis + model terlatih disubmit
# "Training failed" = semua retry crash → base model + noise 0.01 → loss sangat tinggi
grep -q "Training successfully done" "$FULL_LOG" \
    && _check "Training success path (bukan fallback noise 0.01)" "ok" "" \
    || _check "Training success path (bukan fallback noise 0.01)" "fail" "tidak ada 'Training successfully done' — kemungkinan OOM atau crash"

# 8. loss.txt ada di submission (hanya ditulis saat success path)
SUBMIT_DIR="$CACHE_DIR/checkpoints/$TASK_ID/$REPO_NAME"
[ -f "$SUBMIT_DIR/loss.txt" ] \
    && _check "loss.txt ada (eval loss tercatat)" "ok" "" \
    || _check "loss.txt ada (eval loss tercatat)" "fail" "tidak ada loss.txt — training gagal sebelum eval pertama"

# 9. Submission dir ada dan berisi file (model berhasil disimpan)
...
# 10. config.json ada di submission
